...
const Mutations = {
  ...
  async updateItem(parent, args, ctx, info) {
    ...
    // TODO check if they have the permissions
    if (!ownsItem) {
      throw new Error("You don't have permission to do that!");
    }
    // 3. Take a copy of the updates
    const updates = { ...args };
    // 4. remove id from updates
    delete updates.id;
    // 5. run update method
  ...
  async signin(parent, { email, password }, ctx, info) {
    ...
    if (!user) {
      // The error thrown will be caught on the front end Mutation/Query elements
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    const password = await bcrypt.hash(args.password, 10);
    // 5. Save the new password to the user and remove old resetToken and fields
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    if (cartItem.user.id !== ctx.request.userId) {
      throw new Error("Cheating huhhhhhh");
    }
    //3. Delete cart item
